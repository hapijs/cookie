...
describe( 'scheme', function ( ) {
    ...
    it( 'uses request.auth.credentials if already if set if authentication fails', function ( done ) {
        ...
        server.register( require( '../' ), function ( err ) {
            ...
            server.ext( 'onRequest', function ( request, reply ) {
                //Another strategy has been applied and authenticated the user
            ...
            server.auth.strategy( 'default', 'cookie', true, {
                ...
                validateFunc: function ( request, session, callback ) {
                    ...
                    //Fail auth so we can later check to see that previous credentials were applied by onPreAuth handler

                    return callback( null, false, session );
                }
            } );
            ...
            server.route( {
                ...
                config: {
                    ...
                    handler: function ( request, reply ) {
                        ...
                        expect( request.auth.credentials.alreadySet ).to.equal( true );
                        request.auth.session.ttl( 60 * 1000 );
