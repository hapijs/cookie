    ...
    server.ext('onPreAuth', function (request, reply) {
        ...
        request.auth.session = {
            set: function (session) {
                ...
                var cookieOptions = {};
                if (session.rememberMe && !isNaN(settings.rememberMeFor)) {
                    cookieOptions.ttl = settings.rememberMeFor;
                }
                reply.state(settings.cookie, session, cookieOptions);
            },
            clear: function () {
        ...
        authenticate: function (request, reply) {
            ...
            var validate = function () {
                ...
                if (!settings.validateFunc) {
                    if (settings.updateCookieOnValidate) {
                ...
                settings.validateFunc(session, function (err, isValid, credentials) {
                    ...
                    if (settings.updateCookieOnValidate) {
                        request.auth.session.set(credentials || session);
                    }
