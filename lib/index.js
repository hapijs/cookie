...
internals.CookieAuth = class {
    ...
    clear(key) {
        ...
        h.unstate(settings.cookie.name);
    }
    ...
    ttl(msecs) {
        ...
        h.state(settings.cookie.name, session, { ttl: msecs });
    }
};
...
internals.implementation = (server, options) => {
    ...
    const cookieOptions = Object.assign({}, settings.cookie);       // Shallow cloned
    delete cookieOptions.name;
    server.state(settings.cookie.name, cookieOptions);
    ...
    const scheme = {
        authenticate: async (request, h) => {
            ...
            const validate = async () => {
                ...
                const session = request.state[settings.cookie.name];
                if (!session) {
